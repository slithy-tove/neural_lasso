...
def load_mice(one_hot=False):
    ...
    X = np.genfromtxt(
        "data/cache/MICE/Data_Cortex_Nuclear.csv",
        delimiter=",",
        skip_header=1,
        usecols=range(1, 78),
...
def load_isolet():
    x_train = np.genfromtxt(
        "data/cache/isolet/isolet1+2+3+4.data",
        delimiter=",",
        usecols=range(0, 617),
        encoding="UTF-8",
    )
    ...
    x_test = np.genfromtxt(
        "data/cache/isolet/isolet5.data",
        delimiter=",",
        usecols=range(0, 617),
        encoding="UTF-8",
    )
...
def load_activity():
    x_train = np.loadtxt(
        os.path.join("data/cache/dataset_uci", "final_X_train.txt"),
        delimiter=",",
        encoding="UTF-8",
    )
    x_test = np.loadtxt(
        os.path.join("data/cache/dataset_uci", "final_X_test.txt"),
        delimiter=",",
        encoding="UTF-8",
    )
    y_train = (
        np.loadtxt(
            os.path.join("data/cache/dataset_uci", "final_y_train.txt"),
            delimiter=",",
            encoding="UTF-8",
        )
        - 1
    ...
    y_test = (
        np.loadtxt(
            os.path.join("data/cache/dataset_uci", "final_y_test.txt"),
            delimiter=",",
            encoding="UTF-8",
        )
        - 1
...
def load_lassonet_dataset(dataset):
    if dataset == "MNIST":
        return load_mnist()
